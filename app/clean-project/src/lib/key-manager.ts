...
export class ApiKeyManager {
  ...
  public registerRequest(key: string): void {
    ...
    if (Date.now() - this.usage[key].windowStart > RATE_LIMIT_WINDOW_MS) {
      ...
      this.usage[key] = {
        count: 0,
        windowStart: Date.now(),
        consecutiveErrors: this.usage[key].consecutiveErrors,
        lastErrorCode: this.usage[key].lastErrorCode
      };
    }
  ...
  /**
   * Register an error for a specific key
   */
  public registerError(key: string, statusCode?: number): void {
    if (!this.keys.includes(key)) {
      const error = `Unknown API key: ${key.substring(0, 8)}...`;
      console.error(`[KEY MANAGER] ${error}`);
      throw new Error(error);
    ...
    this.usage[key].lastErrorCode = statusCode;
    this.usage[key].consecutiveErrors += 1;
    
    if (DEBUG_LOGGING) {
      console.log(`[KEY MANAGER] Key ${key.substring(0, 8)}... had an error (status: ${statusCode || 'unknown'}, consecutive errors: ${this.usage[key].consecutiveErrors})`);
    }
  }
  
  /**
   * Reset error tracking for a key after a successful request
   */
  public registerSuccess(key: string): void {
    ...
    if (this.usage[key].consecutiveErrors > 0 && DEBUG_LOGGING) {
      console.log(`[KEY MANAGER] Resetting consecutive errors for key ${key.substring(0, 8)}... (was ${this.usage[key].consecutiveErrors})`);
    }
    
    this.usage[key].consecutiveErrors = 0;
  }
  ...
  /**
   * Check if any keys have encountered 403 errors
   */
  public hasForbiddenErrors(): boolean {
    return this.keys.some(key => this.usage[key].lastErrorCode === 403 || this.usage[key].consecutiveErrors > 5);
  }
